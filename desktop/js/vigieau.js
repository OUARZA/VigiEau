...
var vigieauUsageFilterManager = {
  ...
  applySelection: function (selectedKeys) {
    ...
    $container.find('.usage-filter-checkbox').each(function () {
      ...
      for (var s = 0; s < optionKeys.length; s++) {
        if (!vigieauUsageFilterManager.isThematicKey(optionKeys[s])) {
          hasSpecificKey = true;
          break;
        }
      }
      ...
      if (hasStoredSelection) {
        ...
        for (var j = 0; j < optionKeys.length; j++) {
          ...
          if (hasSpecificKey && vigieauUsageFilterManager.isThematicKey(candidateKey)) {
            continue;
          }
  ...
  syncHiddenFromCheckboxes: function (isInitial, initialKeys) {
    ...
    $container.find('.usage-filter-checkbox').each(function () {
      ...
      var primary = vigieauUsageFilterManager.getPrimaryKeyFromCheckbox($checkbox, keys);
      if (primary === '') {
        return;
      }
...
$(document).on('click', '.eqLogicAction[data-action=save]', function () {
  vigieauUsageFilterManager.schedulePostSaveRefresh();
});
